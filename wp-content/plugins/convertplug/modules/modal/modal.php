	...
	class Smile_Modals extends Convert_Plug{
		...
		function enqueue_admin_scripts($hook) {
			...
			if( ( isset( $_GET['hidemenubar'] ) && $_GET['module'] == 'modal' )
				 || ( isset($_GET['style-view']) && $_GET['style-view'] == 'new' && $hook == 'convertplus_page_smile-modal-designer' ) ) {
            	...
            	wp_register_script( 'smile-modal-common', plugins_url( 'assets/js/modal.common.js', __FILE__), array( $handel ), null, true );
		...
		function enqueue_front_scripts(){
			...
			if( $live_styles && count($live_styles) > 0 ) {
					...
					if( isset( $data['cp-dev-mode'] ) && $data['cp-dev-mode'] == '1' ) {
						...
						wp_enqueue_style( 'cp-social-icon-dev-style', plugins_url( '../assets/css/social-icon.css', __FILE__ ) );
