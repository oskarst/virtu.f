	...
	function cp_get_modal_image_url_init( $a = '' ) {
		...
		if( isset( $a['modal_img_src'] ) && $a['modal_img_src'] == 'custom_url' ) {
			...
			if ( strpos($a['modal_image'],'http') !== false ) {
		   	...
		   	$modal_image = cp_get_protocol_settings_init($modal_image);
		} else {
			$modal_image = '';
		}
	   	return $modal_image;
...
<?php } ?>
	<div data-form-layout="<?php echo $form_layout; ?>" class="cp-module cp-modal-popup-container <?php echo esc_attr( $style_id ); ?> <?php echo $style_class. '-container'; ?><?php echo ( $isInline ) ? " cp-inline-modal-container" : ""; ?>">
		<div class="<?php echo ( $isInline ) ? "cp-modal-inline" : "cp-overlay "; ?><?php echo esc_attr( $close_modal_on ); ?> <?php echo esc_attr( $overlay_effect ); ?> content-<?php echo $a['uid'] . ' ' . $global_class . ' ' . $close_class ; ?> <?php echo $impression_disable_class;?>" data-placeholder-font="<?php echo $placeholder_font; ?>" data-class="content-<?php echo $a['uid']; ?>" style=" <?php echo esc_attr( 'background:'.$a['modal_overlay_bg_color'] ); ?>" <?php echo $global_modal_settings; ?> data-custom-class="<?php echo esc_attr( $custom_class ); ?>" data-load-on-refresh="<?php echo esc_attr($load_on_refresh); ?>" <?php echo $isScheduled; ?> data-timezone="<?php echo esc_attr($timezone); ?>" data-timezonename="<?php echo esc_attr( $timezone_name );?>" data-placeholder-color="<?php echo $placeholder_color; ?>" data-image-position="<?php echo $image_position ;?>" <?php echo $hide_image; ?> <?php echo $afl_setting; ?> <?php echo $overaly_setting;?> <?php echo $data_redirect;?> data-tz-offset="<?php echo $schedular_tmz_offset ;?>" <?php echo esc_attr( $close_btn_on_duration ); ?>  <?php echo $autoclose_data; ?>  <?php echo esc_attr( $form_data_onsubmit );?>>
...
function cp_modal_global_after_init( $a ) {
	...
	if( is_user_logged_in() ) {
		// if user has access to ConvertPlus, then only display edit style link
