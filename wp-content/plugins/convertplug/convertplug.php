<?php
/*
Plugin Name: The7 ConvertPlus
Plugin URI: https://www.convertplug.com/plus
Author: Brainstorm Force
Author URI: https://www.brainstormforce.com
Version: 3.0.0
Description: Welcome to ConvertPlus - the easiest WordPress plugin to convert website traffic into leads. ConvertPlus will help you build email lists, drive traffic, promote videos, offer coupons and much more!
Text Domain: smile
*/
...
if( !defined( 'CP_PLUS_NAME' ) ){
	define( 'CP_PLUS_NAME', 'ConvertPlus' );
}

register_activation_hook( __FILE__, 'on_cp_activate' );
add_action( 'the7_theme_activation_action', array('Convert_Plug', 'the7_handle_brainstorm_activation'), 30 );
add_action( 'after_switch_theme', array('Convert_Plug', 'the7_handle_brainstorm_activation'), 30 );
/**
...
function on_cp_activate() {
	Convert_Plug::the7_handle_brainstorm_activation();
...
if(!class_exists( 'Convert_Plug' )){
	...
	class Convert_Plug extends Smile_Framework{
		...
		*/
		...
		/**
		* Add ConvertPlus access capabilities to user roles
		* @since 2.2.0
		*/
		...
		function cp_action_links( $actions, $plugin_file ) {
			...
			if ( $plugin == $plugin_file ) {
				$settings = array('settings' => '<a href="' . admin_url( 'admin.php?page=convertplus&view=settings' ) . '">Settings</a>');
		...
		*/
		function cp_admin_scripts($hook) {
			...
			if( $hook == 'convertplus_page_contact-manager' ) {
			...
			if( !isset( $_GET['hidemenubar'] ) && strpos( $hook , 'convertplus' ) !== false ) {
		...
		*/
		...
		/**
		* Add main manu for ConvertPlus
		* @since 1.0
		*/
		function add_admin_menu(){
			$page = add_menu_page( 'The7 ' . CP_PLUS_NAME . ' Dashboard', CP_PLUS_NAME, 'access_cp', 'convertplus', array($this,'admin_dashboard'), 'div' );
		...
		function add_admin_menu_rename(){
			global $menu, $submenu;
			if( isset( $submenu['convertplus'][0][0] ) ) {
			    $submenu['convertplus'][0][0] = 'Dashboard';
			}
		...
		function cp_submenu_order($menu_ord) {
		    ...
		    if(!isset($submenu['convertplus']))
		    	return false;

		    $temp_resource = $temp_connect = $temp_google_font_manager = $temp_font_icon_manager = $temp_in_sync = $temp_knowledge_base = array();
		    foreach ($submenu['convertplus'] as $key => $cp_submenu) {
		    	if($cp_submenu[2] === 'cp-resources') {
		    		$temp_resource = $submenu['convertplus'][$key];
		    		unset($submenu['convertplus'][$key]);
		    	}
		    	if($cp_submenu[2] === 'contact-manager') {
		    		$temp_connect = $submenu['convertplus'][$key];
		    		unset($submenu['convertplus'][$key]);
		    	}
		    	if($cp_submenu[2] === 'bsf-font-icon-manager') {
		    		$temp_font_icon_manager = $submenu['convertplus'][$key];
		    		unset($submenu['convertplus'][$key]);
		    	}
		    	if($cp_submenu[2] === 'bsf-extensions-14058953') {
		    		$temp_addons = $submenu['convertplus'][$key];
		    		unset($submenu['convertplus'][$key]);
		    	}
		    	if($cp_submenu[2] === 'bsf-google-font-manager') {
		    		$temp_google_font_manager = $submenu['convertplus'][$key];
		    		unset($submenu['convertplus'][$key]);
		    	}
		    	if($cp_submenu[2] === 'knowledge-base') {
		    		$temp_knowledge_base= $submenu['convertplus'][$key];
		    		unset($submenu['convertplus'][$key]);
		    	}
		    	if($cp_submenu[2] === 'cp-wp-comment-form') {
		    		$temp_wp_comment_form = $submenu['convertplus'][$key];
		    		unset($submenu['convertplus'][$key]);
		    	}
		    	if($cp_submenu[2] === 'cp-wp-registration-form') {
		    		$temp_wp_registration_form = $submenu['convertplus'][$key];
		    		unset($submenu['convertplus'][$key]);
		    	}
		    	if($cp_submenu[2] === 'cp-woocheckout-form') {
		    		$temp_woocheckout_form = $submenu['convertplus'][$key];
		    		unset($submenu['convertplus'][$key]);
		    	}
		    	if($cp_submenu[2] === 'cp-contact-form7') {
		    		$temp_contact_form7 = $submenu['convertplus'][$key];
		    		unset($submenu['convertplus'][$key]);
		    	}

		    }

		    array_filter($submenu['convertplus']);

	    	if(!empty($temp_resource)) {
	    		array_push($submenu['convertplus'], $temp_resource);
	    	}
	    	if(!empty($temp_connect)) {
	    		array_push($submenu['convertplus'], $temp_connect);
	    	}
	    	if(!empty($temp_addons)) {
	    		array_push($submenu['convertplus'], $temp_addons);
	    	}
	    	if(!empty($temp_google_font_manager)) {
	    		array_push($submenu['convertplus'], $temp_google_font_manager);
	    	}
	    	if(!empty($temp_knowledge_base)) {
	    		array_push($submenu['convertplus'], $temp_knowledge_base);
	    	}
	    	if(!empty($temp_font_icon_manager)) {
	    		array_push($submenu['convertplus'], $temp_font_icon_manager);
	    	}
	    	if(!empty($temp_wp_comment_form)) {
	    		array_push($submenu['convertplus'], $temp_wp_comment_form);
	    	}
	    	if(!empty($temp_wp_registration_form)) {
	    		array_push($submenu['convertplus'], $temp_wp_registration_form);
	    	}
	    	if(!empty($temp_woocheckout_form)) {
	    		array_push($submenu['convertplus'], $temp_woocheckout_form);
	    	}
	    	if(!empty($temp_contact_form7)) {
	    		array_push($submenu['convertplus'], $temp_contact_form7);
	    	}
		...
		/**
		* Load scripts and styles on admin area of ConvertPlus
		* @since 1.0
		*/
		...
		*/
		function cp_admin_footer() {
			echo'<div id="wpfooter" role="contentinfo" class="cp_admin_footer">
				        ...
				        <span id="footer-thankyou">Thank you for using <a href="https://www.convertplug.com/plus" target="_blank" >' . CP_PLUS_NAME . '</a>.</span>   </p>
		...
		*/
		...
		function cp_add_core_styles($hooks) {
		    ...
		    $contactsPage_hook = 'convertplus_page_contact-manager';
		    $cpmainPage_hook = 'toplevel_page_convertplus';
		...
		*/
		function cp_redirect_to_kb() {
			...
			?><script type="text/javascript">window.location.replace('<?php echo admin_url(); ?>admin.php?page=convertplus&view=knowledge_base'); </script><?php
		}
		...
		*/
		function load_frosty_scripts_from_core($hooks) {
			...
			$array = array(
				'toplevel_page_convertplus',
				'convertplus_page_smile-modal-designer',
				'convertplus_page_smile-info_bar-designer',
				'convertplus_page_smile-slide_in-designer',
				'convertplus_page_contact-manager',
				'convertplus_page_role-manager',
				'admin_page_cp_customizer',
				'convertplus_page_cp-wp-registration-form'
			);
		...
		*/
		function cp_redirect_on_activation(){
			...
			if( get_option('convert_plug_redirect') == true ) {
				...
				if(!is_multisite()) :
					wp_redirect(admin_url('admin.php?page=convertplus'));
				endif;
			}
		}

		public function brainstorm_as_theme() {
			if(defined('BRAINSTORM_THEME_ACTIVATED_URL')) return;
			...
			if ( file_exists( $js_composer_bundled ) ) {
				...
				if ( $code = $bundled_plugin->isActivatedByTheme() ) {
					...
					define('BRAINSTORM_THEME_ACTIVATED_URL', 'https://repo.the7.io/bundled-content/brainstorm/');
				}
			}
		}
			...
			if ( function_exists( 'presscore_theme_is_activated' ) && presscore_theme_is_activated() ) {
				...
				foreach ( $brainstrom_products as $type => $products ) {
					...
					foreach ( $products as $id => $product ) {
						$isActivate = false;
						if (in_array($id, $BRAINSTORM_PRODUCTS)) $isActivate = true;
						else if ( !array_key_exists( "status", $product ) || (array_key_exists( "status", $product ) && ( $product['status'] !== 'registered' )) ) {
		...
		*/
			        	...
			        	if( is_array($scripts) ) {
				        	foreach ($scripts as $key => $script) {
				        		...
				        		if( ( strpos( $source, "wp-content/plugins" ) && !strpos( $source, "wp-content/plugins/". CP_DIR_NAME ) ) || strpos( $source, "wp-content/themes" ) ) {
				        			...
				        			if( isset( $script->handle ) && ( $script->handle !== 'bsf-core-frosty') ) {
		...
		function cp_dequeue_script_imagify() {
			if( isset( $_GET['page'] ) ){
...
function cp_bsf_core_style_hooks($hooks) {
	$resources_page_hook = 'convertplus_page_cp-resources';
...
if( !function_exists( 'cp_rebrand_notice' ) ) {
	function cp_rebrand_notice() {
	    ...
	    <div class="notice notice-warning cp-rebranding-warning is-dismissible">
		    ...
		    <p><?php echo sprintf( __( 'Howdy! ConvertPlug is now rebranded as ConvertPlus. Please read the announcement <a target="_blank" href="%s" >here</a>.', 'smile' ), $link ); ?>
		    </p>
	    </div>
	    <?php
	}
