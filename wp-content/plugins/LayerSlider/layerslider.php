...
define('LS_REPO_BASE_URL', 'https://repo.the7.io/bundled-content/layerslider/');

define('LAYERSLIDER_THE7', true);
layerslider_as_theme();

function layerslider_as_theme() {
	if(!defined('LAYERSLIDER_THE7')) return;
	$theme_path = get_template_directory();
	$layerslider_bundled = "$theme_path/inc/mods/bundled-content/includes/layerslider/layerslider.class.php";
	if ( file_exists( $layerslider_bundled ) ) {

	}
	else{
		if ( get_option( 'layerslider-activated_by_the7', false ) !== 'false' ){
...
add_action('admin_init', 'layerslider_as_theme');
layerslider_as_theme();
